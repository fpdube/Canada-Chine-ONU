...
PM2 <- PM |> filter(debut >= 1984) |> mutate (parti = factor(parti)) |> arrange(debut, fin)

group.colors = c(Libéral = "indianred", Conservateur = "dodgerblue")
...
fonction_enjeux <- function(df, .enjeu) {
  ...
  df %>%
    ...
    geom_smooth(aes(year,!!sym(str_c("prop_", .enjeu))),
                colour = "gray") +
    geom_line(aes(year,!!sym(str_c("prop_", .enjeu))),
              size = 0.8,
              alpha = 0.8,
              colour = "black") +
    ...
    scale_x_continuous(breaks = PM2$debut) +
    # Pour couper le graphique sans perdre de données: coord_cartesian
    coord_cartesian(xlim = c(1990, 2018)) +
...
Regroupement des pays à l'ONU d'après leurs habitudes de vote | Quatre années de données: de l'élection de 2015 à 2018
=============================================================
...
**Groupe 1: le plus progressiste** 

 - 84 pays incluant plusieurs de nos partenaires économiques ou de développement.
 - Majorité de l'Afrique, 
 - Mongolie, Indonésie  
 - Quelques pays développés (Golfe)
 
**Groupe 3: le plus militant**

 -  16 pays incluant Chine, Cuba, Corée du Nord, Russie, Iran, Inde, etc.
 
**Groupe 4: un groupe moins radical** 

 - 26 pays, dont plusieurs SIDS, aux positions plus conservatrices sur le colonialisme et le Moyen-Orient
...
**Groupe 8: Pays en développement proches de la position canadienne**
...
Le Canada est proche de la position des autres pays du CAD (Groupe 5) sur trois enjeux | Il est également proche du Groupe 8
------------------------------------------------------------
...
fn_graphique_boxplot <- function(df, .enjeu) {
  ...
  graphe <- df %>%
    ...
    geom_jitter(heigth = 0.2, width = 0.2,
                alpha = 0.2, size = 1.5, color = "black") +
...
La position canadienne sur le Moyen-Orient et les droits de la personne est alignée sur le Groupe 9 | Le Groupe 8 partage la position canadienne sur le colonialisme
----------------------------------------
