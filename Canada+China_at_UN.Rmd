---
title: "Performance du Canada à l'ONU par rapport à divers ensembles significatifs de pays"
...
Le Canada est devenu le second pays le plus conservateur du G7
---------------------------------------------------------------
...
La question moyen-orientale tire le Canada vers le bas
------------------------------------------------------

Cette situation change-t-elle en fonction des enjeux? Quelques observations émergent de la série de graphiques ci-dessous.

- La Chine continue de voter plus fréquemment en faveur des résolutions de l'AGNU que les pays du G7, et ce, pour tous les enjeux. À l'inverse, les États-Unis sont le pays le plus conservateur des huit.
...
```{r irlande_norvege, fig.width=8, fig.height=16}
...
fonction_enjeux2 <- function(df, .enjeu){
  # On crée trois td: un pour IRL/NOR/CAN; LMIC; UMIC, qu'on joint ensemble
  td_riches <- df %>% 
    filter(!! sym(.enjeu) == 1) %>% 
    group_by(year = year(date), pays) %>% 
    mutate(prop_oui = mean(vote_long == "Pour")) %>% 
  ...
  td_lmic <- df %>% 
    ...
    mutate(pays = "LMIC", pays_long = "LMIC") %>% 
    distinct(year, .keep_all = TRUE)
  ...
  td_umic <- df %>% 
    ...
    # Une fois prop_oui calculé, on simplifie le tableau
    filter(vote_long == "Pour") %>% 
    mutate(pays = "UMIC", pays_long = "UMIC") %>% 
    distinct(year, .keep_all = TRUE)
  
  # On combine les trois td en un seul
  td <- rbind(td_riches, td_lmic, td_umic)
