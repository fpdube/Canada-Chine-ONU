...
theme_set(theme_few()) # Définit le thème utilisé pour les graphiques
...
onu_large %>%
  ...
  ggtitle(
    ...
    "Le Canada est second pays le plus consverateur du G7 à l'ONU"
  ) +
  gghighlight() +
...
**La Chine, le Japon et les États-Unis ont une position stable l'un par rapport à l'autre sur chaque enjeu.** La Chine vote plus souvent en faveur des résolutions de l'ONU que tous les pays du G7 confondus, indépendamment des enjeux. À l'inverse, les États-Unis demeurent le pays le plus conservateur du G7 pour tous les enjeux et presque toutes les années depuis 2000.   
...
patchwork::wrap_plots(graphes_g7[1:3], ncol = 1) # Premier graphique
```

```{r, fig.width=6, fig.height=16}
patchwork::wrap_plots(graphes_g7[4:6], ncol = 1) # Deuxième graphique
```
...
```{r irlande_norvege, fig.width=6, fig.height=16}
...
patchwork::wrap_plots(graphes_IRL_NOR, ncol = 1) + plot_layout(guides='collect')
```

Regroupement de votes
====================
...
regroupements_k <- 
  tibble(k = 1:9) %>% 
  mutate(kclust = map(k, ~ kmeans(td_large, .x)),
...
resolutions_sommaire <- resolutions %>%
  ...
  pivot_longer(cols = c(moyen_orient, nucleaire, desarmement, dev_econ, 
                        droit_pers, colonialisme), names_to = "enjeu", values_to = "nb")
