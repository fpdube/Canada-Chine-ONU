...
Voici un échantillon du tableau `onu_large`. On y a créé un enjeux "autre" pour identifier les situations où une résolution n'entre dans aucune autre catégorie.
...
enjeux <- c("moyen_orient", "nucleaire", "desarmement","droit_pers", "colonialisme", "dev_econ")
...
onu_large <- completeVotes %>% 
  ...
  select(resid, member, vote, vote_long, year, importantvote, short, descr, moyen_orient, nucleaire, 
         desarmement, droit_pers, colonialisme, dev_econ, autre, nb_enjeux, -rcid, everything())

slice_sample(onu_large, n = 3) %>% kable()
```

Le tableau a une taille `r dim(onu_large)` rangées x colonnes.

On reformule le tableau `onu_large` au long (`onu`). 
```{r onu_long}

onu <- onu_large %>% 
  pivot_longer(cols = c(enjeux, autre), names_to = "enjeux", values_to = "valeur") %>% 
  filter(valeur == 1) %>% select(-valeur)

onu %>% slice_sample(n = 3) %>% kable()

```

Le tableau `onu`fait maintenant `r dim(onu)` lignes x colonnes.
