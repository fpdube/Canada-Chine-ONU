...
set.seed(4242)
regroupement_final <- kmeans(td_large, centers = 6, nstart = 100) %>%
  augment(., td_large) %>%
  rename(
    pays = .rownames,
    cluster = .cluster
...
td_corresp <-
  tibble(
    ...
    Groupe = as.character(1L:6L)
  ) %>%
  ungroup()

td_groupe_pays <- left_join(td_groupe_pays, td_corresp, by = "cluster")
onu_final <- left_join(onu_final, td_corresp,
  x.by = "cluster",
  y.by = "cluster"
)
...
graphe_kmeans <-
  map(~ fn_graphe_kmeans(.var = .x), .x = enjeux$var)

titre <- c("Le Canada pourrait considérer se rapprocher des groupes 1, 2 ou 3 pour gagner en influence")
sous_titre <- paste0(
...
patchwork::wrap_plots(graphe_kmeans, ncol = 2) +
