...
Le tableau contient `r onu_large %>% pull(resid) %>% unique() %>% length()` résolutions distinctes depuis 1946, réparties en `r dim(onu_large)[1]` enregistrements.  
...
Nous avons regroupé les pays selon leurs habitudes de votes entre 2005 et 2018 d'après un algorithme de regroupement (*clustering en anglais*). D'après le *critère d'information de Bayes*, le nombre optimal de regroupements est de six.

Quelques particularités à noter:

- **La grande majorité des pays en développement votent à l'AGNU de façon assez homogène**. Il s'agit du groupe 1 dans la série de graphiques ci-dessous, qui comprend 122 pays, incluant les BRICS. Les enjeux du Moyen-Orient et du nucléaire sont à toute fin pratique les seuls où existe une certaine variabilité dans leur vote.

** Un second groupe de pays en développement se distingue par son attitude plus conservatrice sur tous les enjeux.** Il s'agit du groupe 4, composé de dix pays d'Afrique subsaharienne continentale et de six petits États insulaires en développement (PEID). 
...
gmm_res <- Mclust(td_large_gmm, G = 6) %>% 
...
```{r liste_pays_par_cluster}

liste_groupe_pays <- onu_final %>%
  arrange(pays_long) %>%
  arrange(c(cluster)) %>%
  select(c(pays_long, cluster)) %>%
  pivot_wider(id_cols = pays_long, names_from = cluster, values_from = pays_long)

groupe_1 <- liste_groupe_pays[[1]] %>% unlist()
groupe_2 <- liste_groupe_pays[[2]] %>% unlist()
groupe_3 <- liste_groupe_pays[[3]] %>% unlist()
groupe_4 <- liste_groupe_pays[[4]] %>% unlist()
groupe_5 <- liste_groupe_pays[[5]] %>% unlist()
groupe_6 <- liste_groupe_pays[[6]] %>% unlist()

td_groupe_pays <- onu_final %>%
  arrange(pays_long) %>%
  arrange(c(cluster)) %>%
  group_by(cluster) %>%
  select(c(pays_long, cluster)) %>%
  mutate(
    n_cluster = n_distinct(pays_long),
...
td_corresp <-
  tibble(
    cluster = pull(distinct(td_groupe_pays, cluster)),
    Groupe = 1:6
  ) %>%
  ungroup()

td_groupe_pays <- left_join(td_groupe_pays, td_corresp, by = "cluster")
```

### Groupe no. 1: États-Unis, Israël, deux Îles du Pacifique
`r groupe_1`

### Groupe no. 2: Canada, Europe, Japon et Corée du Sud, Australie et Nouvelle-Zélande
`r groupe_2`

### Groupe no. 3: Chine, Corée du Nord, Vénézuela, Iran... et Arabie Saoudite, Inde, Pakistan
`r groupe_3`

### Groupe no. 4: Majorité des pays en développement
`r groupe_4`

### Groupe no. 5: Plusieurs petits États insulaires en développement
`r groupe_5`

### Groupe no. 6: Soudan du Sud
`r groupe_6`
...
fn_impr_tableau2018 <- function(.enjeu) {
  enjeux %>%
    filter(court == .enjeu) %>%
    pull(long) %>%
    print()
...
map(~ fn_impr_tableau2018(.x), .x = enjeux$court)
```
